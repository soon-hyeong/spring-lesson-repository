...
public class Spring10LoggingApplication implements CommandLineRunner {
	...
	public void run(String... args) throws Exception {
		testLogging();
		String id = "java";
		id = "spring"; //test위해 다른 아이디를 입력한다
		simulateLoginService(id);
	}

	public void testLogging(String... args) {
		// 가장 낮은 단계도 테스트
	...
	private void simulateLoginService(String id) {
		/*
		 * {}를 이용한 파라미터 바인딩 -> 성능상 유리 (String + 연산자 사용하는 것 보다) 내부적으로 String Builder 를
		 * 이용, 또한 실행 로깅 레벨이 아니면 문자열 결합하지 않음
		 */
